...
function Episode() {
  ...
  const getSeasons = (movieObj) =>
  Object.keys(movieObj).filter((key) => key !== "video");
const [selectdSeason, setSeletedSeason] = useState(
  getSeasons(movie[movieName[0]])[0]
);
const handleMovieClick = (name) => {
  setSelectedMovie(name);
  setSeletedSeason(getSeasons(movie[name])[0]);
};
    ...
    <>
      <div className="episode-page">
        ...
        <div className="category-bar">
          {movieName.map((name) => (
            <button
              ...
              className={selectedMovie === name ? "active" : ""}
            >
              {name}
            </button>
          ))}
        ...
        <div className="season-bar">
          {getSeasons(movie[selectedMovie]).map((season) => (
            <button
              key={season}
              onClick={() => setSeletedSeason(season)}
              className={selectdSeason === season ? "active" : ""}
            >
              {season}
            </button>
          ))}
        ...
        <div className="episode-container">
          <div className="episode-grid">
            {movie[selectedMovie][selectdSeason].map((ep) => (
              <a
                href={movie[selectedMovie].video}
                target="_blank"
